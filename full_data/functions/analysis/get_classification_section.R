...
## Fitting and plotting classification models for image sections

get_classification_section = function(no_folds, no_repeats, section, treatment, combdata, monodata){
  ...
  balance_tab = table(region_dat$response)
  ...
  for(repeats in 1:no_repeats){
    ...
    cvIndex = caret::createFolds(region_dat$response, k = no_folds, returnTrain = TRUE)
    ...
    for(fold in 1:no_folds){
      ...
      ## Training data
      train_dat = region_dat %>% .[cvIndex[[fold]], ] %>% dplyr::select(-imageID, -batch, -region)
      train_x = train_dat %>% dplyr::select(-response)
      train_y = train_dat %>% pull(response)
      
      ## Testing data
      test_dat = region_dat %>% .[-cvIndex[[fold]], ] %>% dplyr::select(-imageID, -batch, -region)
  ...
  training_acc_p = plot_data_acc %>% 
    ...
    labs(x = "", y = "Accuracy",
         title = paste0(str_to_title(treatment), " Therapy (", section, "): Model Accuracy on Training Data"),
  ...
  training_balacc_p = plot_data_balacc %>% 
    ...
    aes(x = method, y = value, fill = method) +
    labs(x = "", y = "Accuracy",
         title = paste0(str_to_title(treatment), " Therapy (", section, "): Model Balanced Accuracy on Training Data"),
         subtitle = paste0("Repeated CV; n = ", no_repeats, ", k = ", no_folds, "\n", balance_tab[2], " responders & ", balance_tab[1], " non-responders")) +
    scale_x_discrete(labels = c("Logistic Regression", "Logistic Regression\n(Lasso)", "RandomForest", "SVM", "XGBoost")) +
    geom_boxplot() +
    # geom_jitter(width = 0.2, size = 0.2) +
