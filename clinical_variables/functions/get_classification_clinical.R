...
## Classification on clinical variables

get_classification_clinical = function(data, no_repeats, no_folds){
  
  
  # Table of response distribution
  
  balance_tab = table(data$response)
  ...
  for(repeats in 1:no_repeats){
    ...
    cvIndex = caret::createFolds(data$response, k = no_folds, returnTrain = TRUE)
    ...
    for(fold in 1:no_folds){
      ...
      ## Training data
      train_dat = data %>% .[cvIndex[[fold]], ] %>% dplyr::select(-patient, -treatment)
      train_dat$site = 
      train_x = train_dat %>% dplyr::select(-response) %>% data.frame()
      train_y = train_dat %>% pull(response)
      
      ## Testing data
      test_dat = data %>% .[-cvIndex[[fold]], ] %>% dplyr::select(-patient, -treatment)
      test_x = test_dat %>% dplyr::select(-response) %>% data.frame()
      ...
      rf_model = suppressWarnings(
        randomForest::randomForest(response ~ ., data = train_dat) # Need to clean names for randomforest to work
      )
      
      
      svm_model = suppressWarnings(
        svm(train_x %>% sapply(function(x) if(class(x) == "factor"){x = as.numeric(x) - 1} else{x = x}) %>% data.frame(), train_y)
      )
      ...
      svm_pred = suppressWarnings(
        predict(svm_model, test_x %>% sapply(function(x) if(class(x) == "factor"){x = as.numeric(x) - 1} else{x = x}) %>% data.frame())
      )
  ...
  training_acc_p = plot_data_acc %>% 
    ...
    labs(x = "", y = "Accuracy",
         title = "Model Accuracy on Training Data",
         subtitle = paste0("Repeated CV; n = 20, k = 5\n", balance_tab[2], " responders & ", balance_tab[1], " non-responders")) +
  ...
  training_balacc_p = plot_data_balacc %>% 
    ...
    labs(x = "", y = "Balanced Accuracy",
         title = "Model Balanced Accuracy on Training Data",
         subtitle = paste0("Repeated CV; n = 20, k = 5\n", balance_tab[2], " responders & ", balance_tab[1], " non-responders")) +
    scale_x_discrete(labels = c("Logistic Regression", "Logistic Regression\n(Lasso)", "RandomForest", "SVM", "XGBoost")) +
    geom_boxplot() +
    # geom_jitter(width = 0.2, size = 0.2) +
