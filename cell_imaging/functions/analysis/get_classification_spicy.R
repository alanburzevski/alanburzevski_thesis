...
get_classification_spicy = function(no_folds, no_repeats, treatment, combdata, monodata){
  
  ## Defining data to use for the rest of the function
  if(treatment == "combination"){
    dat = combdata[!is.na(combdata$condition), ]
  } else if(treatment == "monotherapy"){
    dat = monodata[!is.na(monodata$condition), ]
  }
  ...
  balance_tab = table(dat$condition)
  ...
  for(repeats in 1:no_repeats){
    ...
    cvIndex = caret::createFolds(dat$condition, k = no_folds, returnTrain = TRUE)
    ...
    for(fold in 1:no_folds){
      ...
      ## Training data
      train_dat = dat %>% .[cvIndex[[fold]], ] %>% dplyr::select(-imageID, -subject, -treatment)
      train_x = train_dat %>% dplyr::select(-condition)
      train_y = train_dat %>% pull(condition)
      
      ## Testing data
      test_dat = dat %>% .[-cvIndex[[fold]], ] %>% dplyr::select(-imageID, -subject, -treatment)
      test_x = test_dat %>% dplyr::select(-condition)
      test_y = test_dat %>% pull(condition) 
      ...
      foldid = caret::createFolds(lasso_train$condition, k = 3) %>% unlist() %>% sort() %>% names() %>% substr(., start = 5, stop = 5) %>% as.numeric() # Obtaining foldID from 5-fold CV
      cv_lasso = cv.glmnet(x = lasso_train %>% dplyr::select(-condition) %>% unlist() %>% matrix(nrow = nrow(lasso_train)),
                           y = as.numeric(lasso_train$condition) - 1, 
                           alpha = 1, family = "binomial",
                           foldid = foldid) # Obtaining best lambda using CV
      
      lasso_model = suppressWarnings(
        glmnet(x = lasso_train %>% dplyr::select(-condition) %>% unlist() %>% matrix(nrow = nrow(lasso_train)),
               y = as.numeric(lasso_train$condition) - 1,
      ...
      rf_model = suppressWarnings(
        randomForest::randomForest(condition ~ .,
                                   na.action = na.roughfix, # Handling missing values
      ...
      lasso_pred = suppressWarnings(
        predict(lasso_model, newx = unlist(lasso_test %>% select(-condition)) %>% matrix(nrow = nrow(lasso_test))) %>% 
          {ifelse(. > 0.5, "Responder", "Non-responder")} %>%  # Cutoff at probability =  0.5
          as.factor()
      )
      ...
      svm_pred = suppressWarnings(
        predict(svm_model, test_x,
                na.action = na.exclude) # Handling missing values
      )
      ...
      lasso_conmat[[fold]] = caret::confusionMatrix(data = lasso_pred, 
                                                  reference = lasso_test$condition)
  ...
  training_balacc_p = plot_data_balacc %>% 
    ...
    labs(x = "", y = "Accuracy",
         ...
         subtitle = paste0("Repeated CV; n = ", no_repeats, ", k = ", no_folds, "\n", balance_tab[2], " responders & ", balance_tab[1], " non-responders")) +
    scale_x_discrete(labels = c("Logistic Regression", "Logistic Regression (Lasso)", "RandomForest", "SVM", "XGBoost")) +
    geom_boxplot() +
    # geom_jitter(width = 0.2, size = 0.2) +
    geom_hline(yintercept = 0.5, linetype = "dashed") +
    theme_classic() +
